...
# DesNet_Architecture
pretrained_densenet = tf.keras.applications.DenseNet201(input_shape=(image_size, image_size, 3), weights='imagenet',
                                                        include_top=False)
...
model_D = Model(inputs=pretrained_densenet.input, outputs=model_out)
model_D.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001), loss=categorical_smooth_loss,
                metrics=['accuracy'])


if __name__ == "__main__":
    history = model_D.fit(training_set, validation_data=testing_set, epochs=50)
