...
def plot_ROC(y_true, y_pred, classes=[0, 1, 2, 3], class_labels=None):
    ...
    # Plot all ROC curves
    plt.figure(figsize=(4.2, 3.7), dpi=300)
    plt.plot(
        fpr["micro"],
        tpr["micro"],
    ...
    plt.plot([0, 1], [0, 1], "k--", lw=lw, markersize=15)
    plt.xlim([0.0, 1.05])
    plt.ylim([0.0, 1.05])
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
...
def plot_PrecRec(y_true, y_pred, classes=[0, 1, 2, 3], class_labels=None):
    #y = label_binarize(y, classes=classes)
    n_classes = len(classes)
    y_true = np.array(y_true)
    y_pred = np.array(y_pred)
    ...
    _, ax = plt.subplots(figsize=(4.2, 3.7), dpi=300)
    ...
    for f_score in f_scores:
        ...
        (l,) = plt.plot(x[y >= 0], y[y >= 0], color="gray", markersize=15, alpha=0.2)
    ...
    for i, color in zip(range(n_classes), colors):
        ...
        display.plot(ax=ax, name=f"{class_labels[i]}", color=color)
